...
class Mute(commands.Cog):
    ...
    async def mute(self, ctx, member: discord.Member, time_minutes: float = default_mute_time):
        ...
        if role in member.roles:
            return await ctx.author.send("[**ERROR**] That user is already muted.")  # Checks if the user is already muted
        else:
            ...
            if (
                ...
                embed = discord.Embed(
                    title=f"Unmuted {member.name}", description=f"{member.mention} has been unmuted.", color=mute_color
                )
    ...
    async def unmute(self, ctx, member: discord.Member):
        ...
        if MuteRole.roleId is None:
            ...
            role = discord.utils.get(
                ctx.guild.roles, id=MuteRole.roleId
            )  # Checks for the role in settings.py, sets it to role
        ...
        if member not in ctx.guild.members:
            ...
            await member.remove_roles(role)  # Same checks as mute command, but now will remove the role if the user is muted
    ...
    async def mutes(self, ctx, member: discord.Member):
        ...
        embed = discord.Embed(
            title="Current Mutes", description=f"{member.mention} has `{get_mutes(member.id)}` mutes.", color=mute_color
        )

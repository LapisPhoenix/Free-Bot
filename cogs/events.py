...
class Events(commands.Cog):
    ...
    def get_ratelimit(self, message: Message) -> Optional[int]:
        ...
        return bucket.update_rate_limit()   # Code stolen from https://stackoverflow.com/questions/65940721/cooldown-for-on-message-in-discord-py
    ...
    async def on_member_join(self, member):
        try:
            ...
            print(e)
        if Welcome.on:
            embed = Embed(title=f"Welcome {member.name}!", description=f"Thanks for joining {member.guild.name}!", color=Welcome.color)
            ...
            if ratelimit is None:   # If the user is not ratelimited, add xp
                try:
                    ...
                    if new_level == old_level + 1:   # If the user leveled up, send a message
                        await message.channel.send(Leveling.LEVEL_UP_MESSAGE.format(user=message.author.mention, level=get_level(message.author.id)))
                    else:

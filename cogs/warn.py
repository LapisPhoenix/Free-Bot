...
class Warn(commands.Cog):
    ...
    async def warn(self, ctx, member: discord.Member, *, reason: str = "No reason provided."):
        ...
        if member.id == ctx.author.id:
            ...
            return await ctx.send(
                "You can't warn me.")  # Check if its the author, a member not in the server, or the bot who is trying to be warned...

        add_warn(member.id)

        embed = discord.Embed(title=f"Warned {member.name}", description=f"{member.mention} has been warned.",
                              color=warn_color)
        ...
        if get_warns(member.id) == max_warns_ban:   # If the user has reached the max amount of warnings, ban them.
            ...
            if MuteRole.roleId == None:
                ...
                return await ctx.author.send(f"**{member.name}** has reached **{max_warns_mute}** warnings, but there is no mute role set!")
    ...
    async def rwarn(self, ctx, member: discord.Member):
        ...
        if get_warns(member.id) == 0:   # If the user has no warnings, return this message.
            ...
            embed = discord.Embed(title=f"Removed Warning from {member.name}",
                                  description=f"{member.mention} has had a warning removed.", color=warn_color)
            embed.add_field(name="Warnings", value=f"Total warnings: `{get_warns(member.id)}`")
    ...
    async def warns(self, ctx, member: discord.Member):
        ...
        embed = discord.Embed(title=f"Current Warnings - {member.name}",
                              description=f"{member.mention} has `{get_warns(member.id)}` total warnings.", color=warn_color)

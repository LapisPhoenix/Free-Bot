...
class Kick(commands.Cog):
    ...
    async def kick(self, ctx, member: Member, *, reason: str = "No Reason Provided."):
        ...
        if member.id == ctx.author.id:
            ...
            return await ctx.send(
                "You can't kick me.")  # Check if the user is trying to kick themselves, or the bot. And Checks if the user is in the server.

        await member.kick(reason=reason)   # Kicks the user
        add_kick(member.id)   # Adds a kick to the database

        embed = Embed(title=f"Kicked {member.name}", description=f"{member.mention} has been kicked because {reason}.",
                      color=kick_color)
    ...
    async def kicks(self, ctx, user: User):
        person = await self.bot.fetch_user(user.id)   # Gets the user object from the id, even if the person isn't in the server.
        embed = Embed(title=f"{person.name}'s Kicks", description=f"{person} has `{get_kicks(user.id)}` kicks.",
                      color=kick_color)

...
class Users(commands.Cog):
    ...
    async def adduser(self, ctx,
                      member: discord.Member):  # If for some reason a member didn't get added, you can add them with this command.
        try:
        ...
        embed = discord.Embed(title="Added User", description=f"Added {member.mention} to the database.",
                              color=user_color)
        ...
        for member in guild.members:
            try:
                if member.id == self.bot.user.id:
                    ...
                    if isindatabase(member.id) != True:   # If the user is not in the database, add them. DO NOT REMOVE THIS LINE!!!

...
class Help(commands.Cog):
    ...
    async def help(self, ctx):
        embed = Embed(
            title="Help", description=f"Use `{prefix}help <command>` for more info on a command.", color=help_color
        )
        embed.add_field(
            name="Moderation",
    ...
    async def userinfo(self, ctx):
        ...
        embed.add_field(
            name="Usage", value=f"`{prefix}userinfo <user>` (Note: if no user is specified, it will show your info)"
        )

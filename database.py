...
name = database_name.replace(' ', '_') if database_name.endswith(".db") else database_name.replace(' ', '_') + ".db"   # Sets the name of the database, makes sure it ends with .db
...
def create_database():
    ...
    curs.execute(f"CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, username TEXT, warns INTEGER, mutes INTEGER, kicks INTEGER, bans INTEGER, xp INTEGER, level INTEGER)")   # Creates the table with ID, username, warns, mutes, kicks, bans, and currency 
